...
%%% ------------- début sons artificiels -- partie 1
%%% construction du signal artificiel
...
nper=1000;  %%% nombre de périodes dans le signal voisé

lsig=round(fe/f0*nper); %%% longueur du signal utile(=voisé) (en échantillons)

%%% signal -- il y a 3 signaux artificiels possibles
ccc=1;
if ccc==1 || ccc==2
  %%% --------------------------------> ccc=1 : sinus
...
%%% ------------- fin sons artificiels -- partie 1


%%% variables à garder pour les vrais sons
...
%%% ------------- début sons artificiels -- partie 2
...
if doit==1
  ...
  tana=0.2; %%% taille de la fenêtre d'analyse (en seconde), pour le calcul des spectres 'spe1' et 'spe2'
            %%% => grande ici parce que les signaux sont stationnaires (car artificiels)
            %%% => attention : si 'nper' est trop petit, 'tana' peut devenit trop grand

  figure(3);
  clf;
  grid on;
  hold on;
  %%% attention : si 'nper' devient trop petit, 0.04 risque d'être trop grand
  ...
  grid on;
  hold on;
  %plot(fff,20*log10(spe1));
  %plot(fff,20*log10(spe2),'r');
  plot(fff,spe1);
  plot(fff,spe2,'r');
